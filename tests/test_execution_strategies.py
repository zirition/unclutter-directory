...
class TestInteractiveStrategy(unittest.TestCase):
    ...
    def test_invalid_input_prompts_again(self, mock_input):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_with("Invalid option. Please choose Y(es), N(o), A(ll), or Never.")
    ...
    def test_prompt_message_format(self, mock_input):
        ...
        expected_prompt = f"Do you want to delete {self.test_file}? [Y(es)/N(o)/A(ll)/Never]: "
        ...
        for action in actions:
            with self.subTest(action=action):
                ...
                self.assertFalse(result,
                    f"DryRun should return False for action: {action}")
...
class TestAutomaticStrategy(unittest.TestCase):
    ...
    def test_both_flags_false_logs_warning(self, mock_logger):
        ...
        expected_msg = f"Automatic strategy without delete preference for {self.test_file}"
    ...
    def test_non_delete_actions_always_execute(self):
        ...
        for always_delete, never_delete in [(True, False), (False, True), (False, False)]:
            strategy = AutomaticStrategy(always_delete, never_delete)

            for action in actions:
                with self.subTest(action=action, always_delete=always_delete, never_delete=never_delete):
                    result = strategy.should_execute_action(
                        self.test_file,
                        action,
                        self.test_rule,
                    ...
                    self.assertTrue(result,
                        f"Non-delete action '{action}' should always execute")
