...
def test_move_basic(temp_dir_setup):
    ...
    ActionExecutor(action).execute_action(test_file, temp_dir)
...
def test_compress_forbidden_extensions(temp_dir_setup, caplog):
    ...
    for ext in forbidden_files:
        ...
        file.touch()
        ActionExecutor({"type": "compress", "target": str(temp_dir)}).execute_action(
            file, temp_dir
        )
...
def test_delete_error_handling(temp_dir_setup, monkeypatch, caplog):
    ...
    ActionExecutor({"type": "delete"}).execute_action(test_file, temp_dir)
...
def test_delete_directory(temp_dir_setup):
    ...
    ActionExecutor({"type": "delete"}).execute_action(test_dir, temp_dir)
